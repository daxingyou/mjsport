new Vue({
		...
		$('#myModal').on('shown.bs.modal', function () {
			...
			$("#submit").off("click").on("click",function(e){
				...
				$("#ajaxForm").ajaxSubmit({
					...
					url: cmsUrl2+'lottery/goodsbase/add',
					type:'post',
					dataType:"json"
				});
	...
	methods:{
		...
		editGoods(id){
			...
			_this.nowItem.type = _this.nowItem.type.toLowerCase();
		...
		getShopList(){
			...
			$.getJSON(cmsUrl2+'lottery/baseindex',function(json){
				_this.goodsList = json.baseGoodsList;
			});
		},
		//提交添加商品
		addSubmit(){
		...
		editSubmit(){
			$("#ajaxForm2").ajaxSubmit({
				...
				url: cmsUrl2+'lottery/goodsbase/edit',
				type:'post',
				dataType:"json"
			});
		}
