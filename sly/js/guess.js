...
Guess.prototype={
    renderGuess:function(){
        ...
        $("#league").on("click",function(){
            ...
            $.post(listUrl+'third/match/online',data2,function(json){
                ...
                if(isData){
                    ...
                    var data3={
                        //method:"bet.match.online.history",
                        page:page,
                        pagesize:"10"
                    }
                    $.post(listUrl+'third/bet/matchOnlineHistory',data3,function(json){
                        console.log(json);
                        var   data   = json.data.matches ;
                        console.log(data)
                        var convertArr = converDate(nowDate,lastDate,data,'start_at');
                ...
                //});
                if(isonline){
                    this.myScroll = new IScroll('#wrapper',{click:true,tap:true,probeType: 1000,distY:-432});
                    myScroll.y=0;
                    var isUp = false;
                    var isDown = false;
                    var isType   = 1;
                    myScroll.on('scrollEnd', function(){
                        ...
                        $.getJSON(listUrl+oUrl,data2,function(json, textStatus) {
                            ...
                            var lastDate   = getLastDate();
                            var convertArr = {}
                            var convertArr = converDate(nowDate,lastDate,data.sort(compare2('start_at')),'start_at');
                            var content = '';
                            var transY   = 0;
                            for(key in convertArr){
                                content += '<div class="match_date">'+key+'</div>';
        ...
        $.post(listUrl1+'third/match/netbar ',data,function(json){
                var type = 'netbar';
                if(json.data == null){
                    var data={
                        page:"1",
